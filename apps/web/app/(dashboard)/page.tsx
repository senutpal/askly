...
export default function Page() {
	...
	// Calculate total queries for a mock "efficiency" metric
	const total =
					...
					>
                                ...
                                <div className="p-6">
                                    {files?.page.length === 0 ? (
                                        <div className="text-center py-8">
                                            <p className="text-sm text-muted-foreground">No knowledge loaded.</p>
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                            {files?.page.slice(0, 3).map((file) => (
                                                <div key={file.id} className="flex items-center gap-3 rounded-xl border border-border/50 bg-background/50 p-3 transition-colors hover:bg-accent/50">
                                                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-muted">
                                                        <FileText className="h-4 w-4 text-muted-foreground" />
                                                    </div>
                                                    <div className="min-w-0">
                                                        <p className="truncate text-sm font-medium">{file.name}</p>
                                                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground">{file.type}</p>
                                                    </div>
                                                </div>
                                            ))}
                                            <Link href="/files" className="flex items-center justify-center gap-2 rounded-xl border border-dashed border-border bg-muted/20 p-3 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground">
						...
						>
							<div className="rounded-3xl border border-border/40 bg-card/50 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl">
								<h3 className="mb-6 font-semibold tracking-tight">Quick Actions</h3>
								<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
									...
									<ActionTile
										icon={AudioWaveform}
										title="Voice Engine"
										subtitle={vapiPlugin ? "Connected to Vapi" : "Setup Required"}
										href="/vapi"
                            ...
                            <div className="relative overflow-hidden rounded-3xl bg-neutral-100 dark:bg-neutral-800 p-6 text-primary/80 shadow-lg">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <p className="mb-4 text-sm ">Check Our Technical Documentation</p>
